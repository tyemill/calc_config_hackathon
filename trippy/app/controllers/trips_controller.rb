class TripsController < ApplicationController
  ...
  def set_random(amount, dest, start)
    ...
    set_activities_rand(amount, dest)
  end
  ...
  def set_activities_cheap(amount, dest)
    total = 0
    @activities_cheap = Array.new
    Activities.limit(2 * params[:duration].to_i).order("cost").where("cost <= '%s' and city = '%s'", amount, dest).each do |act|
      break if ((total += act.cost) > amount)
      total += act.cost
      @activities_cheap << act
    end
  end
  ...
  def set_activities_high(amount, dest)
    total = 0
    @activities_high = Array.new
    Activities.limit(2 * params[:duration].to_i).order("cost DESC").where("cost <= '%s' and city = '%s'", amount, dest).each do |act|
      break if ((total += act.cost) > amount)
      total += act.cost
      @activities_high << act
    end
  ...
  def set_activities_rand(amount, dest)
    total = 0
